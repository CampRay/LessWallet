...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        /// <summary>
        /// Get customer by username or email
        /// </summary>
        /// <param name="account">account</param>
        /// <returns>Customer</returns>
        public virtual Customer GetCustomerByUsernameOrEmail(string account)
        {
            ...
            var query = from c in _customerRepository.Table
                        orderby c.Id
                        where c.Username == account||c.Email== account
                        select c;
            var customer = query.FirstOrDefault();
            return customer;
        }
        ...
        public virtual Customer GetCustomerBySearch(string search)
        {
            if (string.IsNullOrWhiteSpace(search))
                return null;
            var query = from c in _customerRepository.Table
                        orderby c.Id
                        where (c.Username == search || c.Email == search)&&c.Active==true&&c.Deleted==false
                        select c;            
