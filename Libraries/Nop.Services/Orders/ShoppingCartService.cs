            ...
            if (validateOutOfStock && !hasQtyWarnings)
            {
                switch (product.ManageInventoryMethod)
                {
                    case ManageInventoryMethod.DontManageStock:
                        ...
                        break;
                    case ManageInventoryMethod.ManageStock://管理跟踪库存
                        {
                            if (product.BackorderMode == BackorderMode.NoBackorders)//不允许用户缺货购买
                            {
                                int maximumQuantityCanBeAdded = product.GetTotalStockQuantity();
                                if (maximumQuantityCanBeAdded < quantity)
                                {
                                    if (maximumQuantityCanBeAdded <= 0)//如果库存数量为0
                                    {
            ...
            bool ignoreNonCombinableAttributes = false)
        {
            ...
            foreach (var a2 in attributes2)
            {
                if (a2.IsRequired)
                {
                    ...
                    {
                        String str = "";
                        if (a2.AttributeControlType == AttributeControlType.TextBox || a2.AttributeControlType == AttributeControlType.MultilineTextbox)
                        {
                            str = _localizationService.GetResource("ShoppingCart.EnterAttribute");
                        }
                        else {
                            str = _localizationService.GetResource("ShoppingCart.SelectAttribute");                            
                        }
                        var notFoundWarning = !string.IsNullOrEmpty(a2.TextPrompt) ?
                            a2.TextPrompt : 
                            string.Format(str, a2.ProductAttribute.GetLocalized(a => a.Name));
