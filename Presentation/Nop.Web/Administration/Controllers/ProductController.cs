...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        private readonly ICouponPictureService _couponPictureService;
        ...
        public ProductController(IProductService productService,
            ...
            ICouponPictureService couponPictureService,
            ...
            IEncryptionService encryptionService)
        {
            ...
            this._couponPictureService = couponPictureService;
        ...
        protected virtual void UpdateLocales(Product product, ProductModel model)
        {
            foreach (var localized in model.Locales)
            {
                ...
                _localizedEntityService.SaveLocalizedValue(product,
                                                               ...
                                                               localized.LanguageId);
                _localizedEntityService.SaveLocalizedValue(product,
                                                               x => x.ManufacturerPartNumber,
                                                               localized.ManufacturerPartNumber,
                                                               localized.LanguageId);
                _localizedEntityService.SaveLocalizedValue(product,
                                                              x => x.UserAgreementText,
                                                              localized.UserAgreementText,
                                                              localized.LanguageId);
        ...
        protected virtual void PrepareProductModel(ProductModel model, Product product, bool setPredefinedValues, bool excludeProperties)
        {
            ...
            if (product != null)
            {
                ...
                model.AddSpecificationAttributeModel.AvailableAttributes = _cacheManager
                    .Get(string.Format(ModelCacheEventConsumer.SPEC_ATTRIBUTES_MODEL_KEY,_workContext.WorkingLanguage.Id), () =>
                    {
                        ...
                        foreach (var sa in _specificationAttributeService.GetSpecificationAttributes())
                        {
                            ...
                            {                                
                                Text = sa.GetLocalized(x => x.Name, _workContext.WorkingLanguage.Id, true, false),//sa.Name,
                ...
                if (model.AddSpecificationAttributeModel.AvailableAttributes.Any())
                {
                    ...
                    foreach (var sao in _specificationAttributeService.GetSpecificationAttributeOptionsBySpecificationAttribute(selectedAttributeId))
                        ...
                        {
                            Text = sao.GetLocalized(x => x.Name, _workContext.WorkingLanguage.Id, true, false),//sao.Name,
            ...
            foreach (var productType in ProductType.WiCoupon.ToSelectList(true).ToList())
            {
                var productTypeId = int.Parse(productType.Value);
                //只添加WiCoupon, WiTicket,WiCard三种产品类型
                if (productTypeId <= 3)
                {
            ...
            if (model.IsLoggedInAsVendor)
            {
                model.AvailableVendors.Add(new SelectListItem {
                    ...
                    Value = _workContext.CurrentVendor.Id.ToString()
                });
            }
            else
            {
            ...
            {
                model.MaximumCustomerEnteredPrice = 0;
                model.UnlimitedDownloads = false;//限制下载次数
                model.MaxNumberOfDownloads = 99;//最大下载次数                                
                model.StockQuantity = 10000;
                model.DisplayStockAvailability = true;//是否显示剩余库存数量
                model.NotifyAdminForQuantityBelow = 1;//库存数量低于多少时通知管理员(Vender)
                ...
                model.HasUserAgreement = true;//有用户协议
                //model.HasSampleDownload = true;//有样例可下载
                //if (model.IsLoggedInAsVendor)
                //{
                //    model.SampleDownloadId= _workContext.CurrentVendor.PictureId;
                //}
                model.IsDownload = true;//是可下载商品
                model.DisableWishlistButton = true;//禁用收藏按钮
        ...
        public virtual ActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //生成优惠卷图片
                //_couponPictureService.SaveCouponWebPicture(product);
                //UpdatePictureSeoNames(product);

                //生成二维码
                //var url = string.Format("{0}Plugins/API/ScanProduct/{1}", _webHelper.GetStoreLocation(), product.Id);                
                _qrCodeService.SaveQRCodePicture(product);

                ////添加优惠卷默认背景色
                //var psaBg = new ProductSpecificationAttribute
                //{
                //    AttributeTypeId = (int)SpecificationAttributeType.Option,
                //    SpecificationAttributeOptionId = 39,
                //    ProductId = product.Id,
                //    CustomValue = null,
                ...
                //    DisplayOrder = 0,
                //};
                //_specificationAttributeService.InsertProductSpecificationAttribute(psaBg);
                ////添加优惠卷默认底纹
                //var psaShading = new ProductSpecificationAttribute
                //{
                //    AttributeTypeId = (int)SpecificationAttributeType.Option,
                //    SpecificationAttributeOptionId = 47,
                //    ProductId = product.Id,
                //    CustomValue = null,
                //    AllowFiltering = false,
                //    ShowOnProductPage = true,
                //    DisplayOrder = 1,
                //};
                //_specificationAttributeService.InsertProductSpecificationAttribute(psaShading);
                //添加卡卷规格属性 商家默认Logo
                ...
                {                   
                    ...
                    {
                        AttributeTypeId = (int)SpecificationAttributeType.CustomImage,
                        SpecificationAttributeOptionId = 50,
                        ProductId = product.Id,
                        CustomValue = vender.PictureId.ToString(),
                        AllowFiltering = false,
                        ShowOnProductPage = true,
                        DisplayOrder = 2,
                    };
        ...
        public virtual ActionResult Edit(int id)
        {
            ...
            AddLocales(_languageService, model.Locales, (locale, languageId) =>
            {
                ...
                locale.UserAgreementText = product.GetLocalized(x => x.UserAgreementText, languageId, false, false);
                locale.ManufacturerPartNumber = product.GetLocalized(x => x.ManufacturerPartNumber, languageId, false, false);
            });
        ...
        public virtual ActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //{
                //    int count = product.ProductPictures.Count;
                //    var productPicturesArr=product.ProductPictures.ToArray();
                //    for (int i = 0; i < count; i++)
                //    {
                //        var productPicture= productPicturesArr[i];
                ...
                //生成二维码
                //var url = string.Format("{0}Plugins/API/ScanProduct/{1}", _webHelper.GetStoreLocation(), product.Id);
                //var url = _encryptionService.ECBEncryptText("product:"+product.ProductTypeId + ":" + product.Id);
            ...
            var productrSpecsModel = productrSpecs
                .Select(x =>
                {
                    var psaModel = new ProductSpecificationAttributeModel
                    {
                        ...
                        AttributeName = x.SpecificationAttributeOption.SpecificationAttribute.GetLocalized(sp => sp.Name, _workContext.WorkingLanguage.Id, true, false),//x.SpecificationAttributeOption.SpecificationAttribute.Name,
                    ...
                    switch (x.AttributeType)
                    {
                        case SpecificationAttributeType.Option:
                            psaModel.ValueRaw = HttpUtility.HtmlEncode(x.SpecificationAttributeOption.GetLocalized(sp => sp.Name, _workContext.WorkingLanguage.Id, true, false));// HttpUtility.HtmlEncode(x.SpecificationAttributeOption.Name);
        ...
        public virtual ActionResult ProductAttributeMappingInsert(ProductModel.ProductAttributeMappingModel model)
        {
            ...
            //注释掉此功能，让属性可以重复添加
            //ensure this attribute is not mapped yet(确定属性还没有添加)
        ...
        [HttpPost]
        public virtual ActionResult ProductAttributeCombinationDeleteSelected(ICollection<int> selectedIds)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            if (selectedIds != null) { 
                var combination = _productAttributeService.GetProductAttributeCombinationById(selectedIds.FirstOrDefault());
                if (combination == null)
                    throw new ArgumentException("No product attribute combination found with the specified id");
                ...
                _productAttributeService.DeleteProductAttributeCombinations(_productAttributeService.GetProductAttributeCombinationByIds(selectedIds.ToArray()).ToList());                
            }
            return new NullJsonResult();
        }
