...
namespace Nop.Admin.Controllers
{
    public partial class SpecificationAttributeController : BaseAdminController
    {
        ...
        public virtual ActionResult OptionEditPopup(int id)
        {
            ...
            //ColorSquaresRgb字段即可以保存颜色值也可以保存上传的图片Id
            if (String.IsNullOrEmpty(sao.ColorSquaresRgb))
            {
                model.EnableColorSquaresRgb = false;
                model.EnableUploadImage = false;
            }
            else
            {
                //判断是否是颜色值，不是则是上传的图片ID
                if (sao.ColorSquaresRgb.StartsWith("#"))
                {
                    model.EnableColorSquaresRgb = true;
                }
                else
                {
                    model.EnableUploadImage = true;
                    model.PictureId = Convert.ToInt32(sao.ColorSquaresRgb);
                }
            }
            ////"Color" value 原来处理颜色值的代码
        ...
        public virtual ActionResult GetOptionsByAttributeId(string attributeId)
        {
            ...
            var result = (from o in options
                          select new { id = o.Id, name = o.GetLocalized(sp => sp.Name, _workContext.WorkingLanguage.Id, true, false),
                                       value=o.ColorSquaresRgb}).ToList();
